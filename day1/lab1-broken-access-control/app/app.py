...
users = {
    1: {"id": 1, "username": "admin", "password": "admin123", "coins": 100, "flag": False},
    2: {"id": 2, "username": "user", "password": "user123", "coins": 10, "flag": False},
}
...
def render_page(title, body, user=None):
    ...
    if user:
        ...
        if user["id"] == 2:
            nav_items.append("<a href='/store'>Store</a>")
...
def profile():
    ...
    flag_msg = f"<p>Flag: <strong>{u['flag']}</strong></p>" if u["id"] == 2 else ""
    admin_hint = ""
    if u["id"] == 1:
        admin_hint = """
        <div class="card">
          <h3>Update USERS coins</h3>
          <form method="post" action="/admin/update_wallet">
            <label>User ID</label>
            <input name="id" type="number" min="1" required>
            <label>New Coins</label>
            <input name="sold" type="number" min="0" required>
    ...
    <div class="grid">
      <div class="card">
        ...
        <p>Coins: <strong>{u['coins']}</strong></p>
        {flag_msg}
      </div>
      <div class="card">
...
def update_password():
    ...
    """
    return render_page("Profile", body, cu), 400
...
def store():
    ...
    if u["id"] != 2:
        ...
        <div class="card">
          <div class="badge">Access Denied</div>
          <h3>Store is for normal user only</h3>
        </div>
        """
        return render_page("Store", body, u), 403

    body = """
    <div class="card">
      <div class="badge">Limited Offer</div>
      <h3>Golden Flag</h3>
      <p class="muted">Price: 100 coins</p>
      <form method="post" action="/buy_flag">
        <button type="submit">Buy</button>
      </form>
    </div>
    """
    return render_page("Store", body, u)
...
def buy_flag():
    ...
    if u["id"] != 2:
        ...
        <div class="card">
          <div class="badge">Access Denied</div>
          <h3>Only normal user can buy</h3>
        </div>
        """
        return render_page("Store", body, u), 403
    ...
    if u["coins"] >= 100:
        u["coins"] -= 100
        u["flag"] = "CONGR4TS_H333333333CKER"
        body = """
        <div class="card">
          <div class="badge">Success</div>
          <h3>Purchased!</h3>
          <p>Flag: <strong>CONGR4TS_H333333333CKER</strong></p>
        </div>
        """
        return render_page("Store", body, u)
    body = """
    <div class="card">
      <div class="badge">Not Enough</div>
      <h3>You need 100 coins</h3>
    </div>
    """
    return render_page("Store", body, u), 400
...
def update_wallet():
    ...
    new_sold = int(data.get("sold", 0) or request.form.get("sold", 0))
    if uid in users:
        users[uid]["coins"] = new_sold
        return jsonify({"status": "ok"}) if request.is_json else redirect(url_for("profile"))
    if request.is_json:
        return jsonify({"status": "fail"}), 400
    body = """
